...
public class EncryptionInterceptor implements Interceptor {
    ...
    private ObjectEncrypt processFields(Object entity, Object[] currentState, String[] propertyNames, String type) {
                ...
                if (propertyNames[i].equals(annotationField)) {
                    if(StringUtils.hasText(currentState[i].toString())) {
                        if("onSave".equals(type) || "OnFlushDirty".equals(type)) {
                            ...
                            System.out.println("Type " +type + " "+encryptionService.encrypt(currentState[i].toString()));
                        }
                        else if("onLoad".equals(type)) {
                            currentState[i] = encryptionService.decrypt(currentState[i].toString());
                            System.out.println("Type " +type + " "+encryptionService.decrypt(currentState[i].toString()));
                        }
                    }
                }
